...
进入 Shadowrocket 的 HTTPS 解密页面，开启 HTTPS 解密和“通过 HTTP/2 进行中间人攻击”。正式模块会自动加入：
...
无需在“指定的域名请求”中手动填写。不要加入高德、阿里 DNS、Cloudflare 后台或地图瓦片域名。
...
正式方案不要求添加 `DIRECT`、`PROXY`、Fake-IP、DNS 或 CNAME 特例。确认手机浏览器可以打开后台域名，并且不要把 Cloudflare 后台域名加入 HTTPS 解密列表。
...
3. CA 证书已完全信任。
4. 已开启 HTTP/2 中间人攻击。
5. 只启用了后台生成的一个 `shadowrocket-v2.sgmodule`，旧定位模块全部删除。

### 8.4 排除模块冲突

只保留 `/admin` 生成的一个正式模块。Apple Test、Static Test、上游基础模块或重复 Plus 模块会拦截同一个 Apple WLOC 响应，可能把后台坐标覆盖成苹果总部。删除全部旧模块后重新导入正式模块。`v1.0.0` 起旧诊断地址返回 `410 Gone`。
...
Do not manually fill the requested-host list. Do not add Amap, AliDNS, Cloudflare dashboard, or map-tile hostnames.
...
The standard setup requires no custom `DIRECT`, `PROXY`, Fake-IP, DNS, or CNAME rule. Confirm the dashboard domain opens in the iPhone browser, and do not add it to the HTTPS decryption list.
...
5. Exactly one generated `shadowrocket-v2.sgmodule` is enabled and every old location module is deleted.

### 8.4 Eliminate Module Conflicts

Keep only one official module generated by `/admin`. Apple Test, Static Test, upstream base, or duplicate Plus modules intercept the same Apple WLOC response and may override the dashboard location with Apple Park. Delete every old module and import the official module again. From `v1.0.0`, legacy diagnostic URLs return `410 Gone`.
