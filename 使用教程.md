...
这是一套免费的 iPhone 定位管理方案：手机上用支持 HTTPS 解密的代理工具拦截 Apple 定位接口，Cloudflare 上部署一个 Plus 后台。后台首次生成 TOKEN，随后自动给出 Shadowrocket / Surge / Loon / Quantumult X / Stash 模块 URL，并直接提供地图定位管理。
...
其他客户端在后台复制对应 URL 后导入：Surge 复制 `surge.sgmodule`，Loon 复制 `loon.lnplugin`，Stash 复制 `stash.stoverride`，Quantumult X 复制 `quantumultx.snippet`。QX 片段使用当前坐标静态生成，后台修改坐标后需要重新导入或刷新片段。

Shadowrocket、Surge、Loon、Stash 更换定位通常不需要重新导入模块，只需要在 Plus 后台地图保存新位置，再关闭并开启一次 iPhone 定位服务。

正式方案不要求添加 `DIRECT`、`PROXY`、Fake-IP、DNS 或 CNAME 特例。确认手机浏览器可以打开 Cloudflare 后台域名，并且不要把后台域名加入 HTTPS 解密列表。
...
打开 HTTPS 解密，并开启“通过 HTTP/2 进行中间人攻击”。正式模块会自动加入：
...
2. HTTPS 解密是否开启。
3. CA 证书是否完全信任。
4. 是否开启了 HTTP/2 中间人攻击。
5. 是否删除全部旧定位模块，只导入并启用了后台生成的一个 Plus 正式模块。
6. 保存后台坐标后是否关闭再开启 iPhone 定位服务。

如果后台是伦敦或香港，手机却固定在苹果总部，几乎可以确定仍有 Apple Test、Static Test、上游基础模块或重复 Plus 模块在抢占响应。删除全部旧定位模块，再从后台重新导入唯一的正式模块。
